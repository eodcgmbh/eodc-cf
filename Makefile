...
test:
	source .venv/bin/activate && pytest tests/ -rsx --verbose --color=yes --cov=eodc_cf --cov-report term-missing

version:
	echo -e "__version__ = \"$(shell git describe --always --tags --abbrev=0)\"\n__commit__ = \"$(shell git rev-parse --short HEAD)\"" > src/eodc_cf/_version.py
